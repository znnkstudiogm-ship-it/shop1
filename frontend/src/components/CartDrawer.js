...
export const CartDrawer = () => {
  ...
  const subtotal = checkout?.subtotalPrice
    ? formatMoney(checkout.subtotalPrice.amount, checkout.subtotalPrice.currencyCode)
    : formatMoney(0, "USD");
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6">
          {items.length === 0 ? (
            ...
            >
              ...
              <p className="text-zinc-400 font-body">Twój koszyk jest pusty.</p>
              <button
                type="button"
                onClick={() => setCartOpen(false)}
            ...
            <ul className="space-y-5">
              {items.map((li) => {
                ...
                const lineTotal = price
                  ? formatMoney(
                      parseFloat(price.amount) * li.quantity,
                      price.currencyCode
                    )
                  : "";
                return (
                      ...
                      <div className="flex items-center justify-between">
                        <div className="inline-flex items-center border border-white/15">
                          <button
                            ...
                            onClick={() =>
                              updateLineItem(li.id, Math.max(1, li.quantity - 1))
                            }
        ...
        {items.length > 0 && (
          <div className="border-t border-white/10 p-6 space-y-4">
            ...
            <p className="text-[10px] text-zinc-500 text-center tracking-wider">
              Podatki i koszty dostawy obliczane w kasie Shopify.
            </p>
          </div>
        )}
      </aside>
